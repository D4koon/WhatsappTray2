...
namespace WhatsappTray2
{
	public class WhatsAppApi
	{
		...
		/// <summary>
		/// From https://github.com/mukulhase/WebWhatsapp-Wrapper/blob/master/webwhatsapi/js/wapi.js
		/// </summary>
		public static string Initialize()
		{
			logger.Info("Initializing WhatsappApi.");
...
window.WAPI.demoteParticipantAdminGroup = function (idGroup, idParticipant, done) {
	window.Store.WapQuery.demoteParticipants(idGroup, [idParticipant]).then(() => {
		...
		if (checkParticipant !== undefined && checkParticipant.isAdmin) {
			...
			if (evaluateTask.Wait(10000) == false) {
				// Timeout
		...
		{
			if (Browser == null) {
				logger.Error("WhatsAppApi.Browser needs to be set before using WhatsAppApi!");
				return false;
			}

			JavascriptResponse javascriptResponse;
			try {
				javascriptResponse = ExecuteScript("if (document.querySelector('*[data-icon=chat]') !== null) { return true } else { return false }", 2000);
				//javascriptResponse = ExecuteScript(Browser, "return window.WAPI.getAllContacts();", 2000);
			} catch (Exception) {
				return false;
			}

			if (javascriptResponse.Result is bool) {
		...
		public static int GetAllChatsWithNewMsgCount()
		{
			JavascriptResponse javascriptResponse;
			try {
			...
			if (javascriptResponse.Result is List<object>) {
				List<object> bla = (List<object>)javascriptResponse.Result;
				return bla.Count();
			}
		...
		public static JavascriptResponse ExecuteScript(string script, int millisecondsTimeout = 2000)
		{
			...
			var evaluateTask = Browser.EvaluateScriptAsync(scriptBuffer);

			if (evaluateTask.Wait(millisecondsTimeout) == false) {
				// Timeout
				throw new Exception("Timeout on Javascript-execution");
			}

			// Da ich das objet nicht so uebertragen kann da c# es nicht serialisieren kann, mach ich das serialisieren selber...
			//return GetJsResultAsString(evaluateTask.Result.Result);
